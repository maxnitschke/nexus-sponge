...
public class NexusZone {
	...
	public NexusZone(String name, String world, double x1, double y1, double z1, double x2, double y2, double z2, double priority, String owner, HashMap<String, Double> members, HashMap<String, String> settings) {
		this.name = name;
		this.world = world;
		this.x1 = x1;
		this.y1 = y1;
	...
	public void insert() {
		NexusDatabase.addZone(name, this);
		NexusDatabase.queue("INSERT INTO zones VALUES ('" + name + "', '" + world + "', " + x1 + ", " + y1 + ", " + z1 + ", " + x2 + ", " + y2 + ", " + z2 + ", " + priority + ", '" + owner + "', '" + SerializeUtils.members(members) + "', '" + SerializeUtils.settings(settings) + "')");
	}
	
	public void update() {
		NexusDatabase.addZone(name, this);
		NexusDatabase.queue("UPDATE zones SET world = '" + world + "', x1 = " + x1 + ", y1 = " + y1 + ", z1 = " + z1 + ", x2 = " + x2 + ", y2 = " + y2 + ", z2 = " + z2 + ", priority = " + priority + ", owner = '" + owner + "', members = '" + SerializeUtils.members(members) + "', settings = '" + SerializeUtils.settings(settings) + "' WHERE name = '" + name + "'");
	}
	
	public void delete() {
		NexusDatabase.removeZone(name);
		NexusDatabase.queue("DELETE FROM zones WHERE name = '" + name + "'");
	}
	...
	public List<NexusZone> getOverlappingZones() {
		
		List<NexusZone> result = new ArrayList<NexusZone>();
		
		for(Entry<String, NexusZone> e : NexusDatabase.getZones().entrySet()) {
			
			NexusZone z = e.getValue(); if(!world.equals(z.getWorld())) continue;
